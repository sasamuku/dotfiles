...
| `Ctrl+e` | エクスプローラーへフォーカス（現在ファイルを選択）。ツリーにフォーカス中に押すと非表示（VSCode風） |

ツリーにフォーカス中のファイル操作（neo-tree デフォルト）:

| キー | 説明 |
|------|------|
| `a` | ファイル作成（末尾を `/` にするとディレクトリ） |
| `A` | ディレクトリ作成 |
| `r` | リネーム |
| `d` | 削除 |
| `y` / `x` / `p` | コピー / カット / ペースト（クリップボード） |
| `c` / `m` | コピー / 移動（パス入力） |
| `R` | ツリー再描画 |
| `H` | 隠しファイル表示切り替え |
...
|------|------|
| `gcc` | カーソル行をコメント切り替え |
| `gc`（Visual） | 選択行をまとめてコメント切り替え |
| `gc{範囲}` | 範囲をコメント切り替え（例: `gc3j`, `gcip`） |
| `gbc` / `gb`（Visual） | ブロックコメント切り替え |
