...
相談文には「X は所与」「Y は既に決まっている」と明示されていない前提が多数含まれる。**相手が方針として書いた動詞 (削除する / 物理削除する / 毎日実行する / 同期する 等) を機械的に抜き出し、各前提に「これは本当に必要か?」を 1 つずつ当てる**。
...
2. 各方針について根拠が示されているか確認 (GDPR 要件 / 仕様書 / 既存コード制約 等)
...
| 判定 | 条件 |
|---|---|
| 強い根拠 | 相談文内に法的要件 (GDPR 等) / 契約 / 既存仕様書 / 物理的制約 / 数値根拠 のいずれかが明示されている |
...
**例**: 「workspace_members を物理削除する」と相談文に書いてあったら、「物理削除」は所与か? 根拠が示されていなければ、「物理削除しない (論理削除 or 残す)」を Step 5 の選択肢に必ず含める。
