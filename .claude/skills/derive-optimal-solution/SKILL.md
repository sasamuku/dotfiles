...
- 小さな相談: Step 1〜3 のみ軽く。Sub Agent は使わない (オーバーヘッド負け)。充足条件 = 次をすべて満たす: 論点が 1 つ / 成果物がその場の回答文のみで完結する / 誤った推奨をしてもすぐ訂正が効く。1 つでも欠けるか判断に迷う場合は中規模に倒す
...
**該当判定は相談文に明示された事実のみでカウントする**: 条件の充足が相談文から読み取れない場合、その条件は非該当と扱う (推測で該当させない)。「迷ったら大規模側」は判定材料が実在して解釈に迷う場合の規則であり、材料の欠落を「迷い」に数えない。「関係者 3 人以上」は不可逆性・専門性を伴う意思決定の場 (PR レビュー・設計レビュー・経営判断等) を指し、定例の参加人数のような日常運用の人数は該当しない。

「明示」の基準 = 条件の対象が相談文中で**名指し**されていること。技術用語や一般名詞からの推測は明示に数えない:

- 影響範囲 2 層以上: 複数の層名・コンポーネント名が書かれているときのみ該当(「API」「エンドポイント」等の単語から DB 層への波及を推測しない)
- 関係者 3 人以上: レビュー・会議体・承認プロセスの名指しがあるときのみ該当(「チーム」「みんな」等の一般名詞は数えない)
- 相談の性質が重い(過去の意思決定の当否評価等)こと自体は大規模の該当条件ではない。形式的に不充足なら中規模以下として扱ってよい

**深さとテンプレートの優先関係**: 小さな相談では、この軽量化ルールが出力テンプレートの節構成 (「省略不可」の文言を含む) に **優先する**。

| 深さ | 出力テンプレートの扱い |
|---|---|
| 小さな相談 | 必須は「症状/構造・上流・前提疑いの要点」「推奨 1 案 + 却下理由」「不確実性」のみ。境界拡張・評価軸・採点表は省略可。反復記録は全問 Yes なら `自答チェック: Q1〜Q7 すべて Yes` の 1 行に畳んでよい (No が出た Q のみ明記) |
| 中規模・大きな判断 | テンプレート全節が必須 (省略不可) |
...
### 相談の型による読み替え: 評価・判定型 (Yes/No 判定・過去判断の妥当性)

相手が対策案ではなく **判定** (「〜で済んだのか」「〜は正しかったか」「A と B どちらが良かったか」) を求めている場合、次を読み替えて適用する。読み替え以外の手順は共通。

**適用範囲は「既に完了した判断・既成事実」の当否評価に限る**。将来の選択 (「A と B どちらにすべきか」「どちらがいいと思う?」) や、これから採る対策への意見 (「〜しようと思うがどう思う?」) は通常の対策型として扱う。例示の「A と B どちらが良かったか」は過去形 = 完了済み判断を指す。

- **Step 2 (上流遡及)**: 遡及先は「症状の直接原因」だけでなく「判断当時の意思決定プロセス (何を基準に・どの代替案と比較して決めたか)」を必ず含める。判断当時の情報が相談文に無い場合は、その欠落自体を「何が分かれば判定が確定するか」として不確実性節に明示する
- **Step 5 (案)**: 案は行動選択肢ではなく「評価の切り口 (どの解釈が最も事実に整合するか)」として生成する。出力テンプレートが必須とする「やらない / 延期」案も、行動 (「延期する」) ではなく解釈 (「現時点の情報では判定を確定できないとする立場」) として表現する — Q1 の行動選択肢混入と区別するため
- **Step 6 (重さ)**: 重さは「その判定に要する検証・情報収集の手間」と読み替える。5 段階の対応: 1 = 既存情報の再解釈のみ / 2 = 既存データの範囲を絞った確認 / 3 = 既存データの追加集計 / 4 = 新規データの収集 / 5 = 過去の記録・合意事項の掘り起こし
- **Step 7 (帰結予測)**: 「相手の提案」が存在しないため、「相手の指摘・主張をそのまま真として採用した場合」の帰結を予測する
- **Q6 (同等効果)**: 解消すべき症状が存在しないため、「同じ確度の判定に到達できる最も軽い評価枠組みか」と読み替える
- **推奨の書き方**: `## 推奨` の先頭行に可否判定の結論を 1 行で明示してから根拠を展開する。判定を左右する検証不能な情報は「何が分かれば判定が確定するか」として明示し、不確実性節にも降ろす
...
1. 相談文から「相談者が採用予定と書いた方針」を語句単位でリストアップ (最低 3 つ。相談文が短く 3 つに満たない場合は存在する分だけで進み、架空の方針を補わない)。抽出対象は **動詞・目的語**(毎日実行する / 同期する / 全件走査する / 事前計算する 等) **+ 副詞・モーダル** (「とりあえず」「一応」「念のため」「様子見で」「念のため」「まずは」 等)
...
**並列化は品質担保の手段であり、必須成果物ではない**。Sub Agent を使えない環境 (自身が subagent として動作中、Agent/Task ツール無効 等) では、該当ステップを省略せず **直列で代替する**: 視点/次元/層ごとに独立したブロックとして書き切ってから次に進み、前のブロックの結論を参照しない (引きずられ防止)。直列代替は正規の実行手順であり、裁量補完として報告する必要はない。
...
**最終出力はこのテンプレートに従う** (小さな相談は「適用の深さ」の深さとテンプレートの優先関係に従って節を省略してよい)。下記チェックリストは内部検証用で出力には含めない。
...
- [ ] 出力テンプレートの固定書式 (`重さ: N`、各節見出し) を生成後の本文と逐一照合したか
