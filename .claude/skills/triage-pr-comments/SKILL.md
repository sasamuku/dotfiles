...
**トリガー**: ユーザーが「対応する価値があるか判定して」「対応すべきか教えて」「価値判定まで含めて」など、priority 分類とは別に「対応すべきか否か」の判断を明示的に要求した場合のみ実行する。明示要求がなければステップ 3 で出力を確定し、このステップは飛ばす。

**前提**: priority (🔴 Must / 🟡 Investigate / 🟢 Info) は **コメントの深刻度** を示し、Value (対応価値) は **このユーザーがこの PR で対応すべきか** を示す独立した軸。両者は一致しない:

- 🔴 Must だが Value = No (例: 変更前から存在する問題、別 PR スコープ)
- 🟢 Info だが Value = Yes (例: 修正コストがほぼゼロの低リスク改善)
...
**判定軸** (各コメントに対し最低 2 軸を明示する):

| 軸 | 観点 | Value を上げる方向 / 下げる方向 |
|---|---|---|
| 実害サイズ | 本番障害・データ損失・セキュリティ侵害が確認 / 想定されるか | 確認済み実害あり → Yes 寄り / 理論上のみ → No 寄り |
| 修正コスト | 修正に要するファイル数・テスト範囲・レビュー負担 | 1 箇所完結 → Yes 寄り / 大規模リファクタ → 保留 |
| 現 PR スコープ適合 | 本 PR の目的・変更差分とこの修正が整合するか | 同一スコープ → Yes / 別スコープ → 保留 (別 PR) |
| 変更前から存在 | この行・この問題は本 PR の差分で導入されたか | 本 PR で導入 → Yes / 既存問題 → No or 保留 |
...
**Details セクションに `Value Reasoning` 行を追加する** (1〜2 文):
...
**重複指摘の統合** (任意): 同一 path + 近接行 (±5 行以内) + 同一趣旨のコメントが複数ある場合、1 つのグループにまとめて Value 判定を 1 回だけ出してよい。代表コメントは「最高 priority かつ最初の行番号」を選び、他は Reviewer 列に併記 (`CodeRabbit / Claude / Devin` のように `/` 区切り)。

**裁量を残してよい箇所**:

- 「実害サイズ」が明示されていないコメントの判定 — 妥当な仮定を 1 文で添える (例: "Sentry に類似エラー記録がない前提")
- 「保留」と「No」の境界 — 別 PR で追跡する価値があれば 🤔 保留、追跡する価値もなければ ❌ No
