...
**ステージング**: 対象ファイルが既に `git add` 済みなら再 add は不要 (`git diff --cached` で確認)。未 staged のファイルだけを追加でステージする。

**メッセージの言語**: `git log --oneline -10` で観察した既存コミットの言語に合わせる (英語が多ければ英語、日本語が多ければ日本語)。混在していれば英語を既定とする。1 つの PR / セッション内では言語を統一する。
